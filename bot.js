...
client.once('ready', async () => {
  console.log(`✅ تم تسجيل الدخول بنجاح باسم: ${client.user.tag}!`);
    ...
    // تحديث البيانات من الملفات عند بدء التشغيل
    // The following function seems to be missing, assuming it's intended to load initial data.
    // If it's a custom function, ensure it's defined elsewhere or integrated here.
    // For now, we rely on the initial global variable assignments.
    // updateBotDataFromFiles();
    ...
    // بدء نظام فحص الداونات المنتهية كل دقيقة
    const downManager = require('./utils/downManager');
    setInterval(async () => {
        try {
            const expiredDowns = downManager.getExpiredDowns();
            if (expiredDowns.length > 0) {
                console.log(`⏰ تم العثور على ${expiredDowns.length} داون منتهي الصلاحية`);
                ...
                for (const expiredDown of expiredDowns) {
                    try {
                        ...
                        const result = await downManager.endDown(
                            ...
                            client,
                            expiredDown.downId,
                            'انتهت فترة الداون تلقائياً'
                        );

                        if (result.success) {
                            console.log(`✅ تم إنهاء داون ${expiredDown.downId} تلقائياً بنجاح`);
                        } else {
                            console.error(`❌ فشل في إنهاء داون ${expiredDown.downId}: ${result.error}`);
                        }
                    } catch (error) {
                        console.error(`❌ خطأ في إنهاء داون ${expiredDown.downId}:`, error);
                    }
                }
            }
        } catch (error) {
            console.error('خطأ في فحص الداونات المنتهية:', error);
        }
    }, 60000); // فحص كل دقيقة

    // فحص فوري للداونات عند بدء التشغيل
    setTimeout(async () => {
        try {
            const expiredDowns = downManager.getExpiredDowns();
            if (expiredDowns.length > 0) {
                console.log(`⏰ تم العثور على ${expiredDowns.length} داون منتهي الصلاحية عند بدء التشغيل`);
                ...
                if (guild) {
                    for (const expiredDown of expiredDowns) {
                        try {
                            ...
                            if (result.success) {
                                console.log(`✅ تم إنهاء داون ${expiredDown.downId} عند بدء التشغيل`);
                            }
                        } catch (error) {
                            console.error(`❌ خطأ في إنهاء داون ${expiredDown.downId}:`, error);
                        }
            ...
            console.error('خطأ في الفحص الأولي للداونات:', error);
        }
    }, 7000); // فحص بعد 7 ثوان من بدء التشغيل
...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    if (interaction.customId && interaction.customId.startsWith('vac_')) {
            ...
            if (action === 'approve') {
                ...
                if (result.success) {
                    const successEmbed = new EmbedBuilder()
                        ...
                        .setDescription(`✅ **تم إنهاء إجازة <@${userId}> بنجاح**`);
        ...
        // النظام القديم للإنهاء المبكر تم نقله إلى my-vacation.js
    }
    ...
    if (interaction.customId && interaction.customId.startsWith('down_')) {
        const downContext = { client, BOT_OWNERS };

        // Route to set-down command for configuration interactions
        if (interaction.customId.includes('_set_') || 
            interaction.customId.includes('_perm_') || 
            interaction.customId.includes('_channel_') ||
            interaction.customId === 'down_set_permissions' ||
            interaction.customId === 'down_set_channels' ||
            interaction.customId === 'down_send_menu' ||
            interaction.customId === 'down_back_main' ||
            interaction.customId.startsWith('down_role_select') ||
            interaction.customId.startsWith('down_responsibility_select') ||
            interaction.customId.startsWith('down_menu_channel_select') ||
            interaction.customId.startsWith('down_log_channel_select')) {
            
            const setDownCommand = client.commands.get('set-down');
            if (setDownCommand && setDownCommand.handleInteraction) {
                await setDownCommand.handleInteraction(interaction, downContext);
            }
            return;
        }

        // Route to down command for main menu and operations (persistent menu system)
        if (interaction.customId.startsWith('down_main_menu') ||
            interaction.customId.startsWith('down_selected_user') ||
            interaction.customId.startsWith('down_role_selection') ||
            interaction.customId.startsWith('down_modal_') ||
            interaction.customId.startsWith('down_show_user_records') ||
            interaction.customId.startsWith('down_modify_selection') ||
            interaction.customId.startsWith('down_show_user_active_downs') ||
            interaction.customId.startsWith('down_end_user_downs_') ||
            interaction.customId === 'down_back_to_main' ||
            interaction.customId === 'down_records_user_select' ||
            interaction.customId === 'down_modify_select' ||
            interaction.customId === 'down_user_downs_select') {
            
            try {
                const downManager = require('./utils/downManager');
                const settings = downManager.getSettings();
                
                // Check if interaction is in the menu channel (if configured) for main menu
                if (settings.menuChannel && interaction.channelId !== settings.menuChannel && interaction.customId === 'down_main_menu') {
                    // Only owners can use the menu outside the designated channel
                    const isOwner = BOT_OWNERS.includes(interaction.user.id);
                    if (!isOwner) {
                        const wrongChannelEmbed = new EmbedBuilder()
                            .setColor('#ff6b6b')
                            .setDescription(`❌ **يجب استخدام المنيو في الروم المحدد!**\n📍 توجه إلى <#${settings.menuChannel}>`);
                        return interaction.reply({ embeds: [wrongChannelEmbed], ephemeral: true });
                    }
                }
                
                // Check permissions for down system
                const hasPermission = await downManager.hasPermission({ 
                ...
                if (!hasPermission) {
                    const noPermEmbed = new EmbedBuilder()
                        .setColor('#ff6b6b')
                        .setDescription('❌ **ليس لديك صلاحية لاستخدام هذا النظام!**');
                    return interaction.reply({ embeds: [noPermEmbed], ephemeral: true });
                }
                
                // Handle the down interaction
                ...
                console.error('خطأ في معالجة تفاعل الداون:', error);
                if (!interaction.replied && !interaction.deferred) {
