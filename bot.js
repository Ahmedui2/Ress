...
client.once(Events.ClientReady, async () => {
  console.log('✅ حدث Ready تم استلامه - بدء تهيئة الأنظمة');
  try {
    // تهيئة كاش الدعوات
    for (const guild of client.guilds.cache.values()) {
        try {
  ...
  console.log(`✅ تم تسجيل الدخول بنجاح باسم: ${client.user.tag}!`);
    ...
    try {
        ...
        // في حالة فشل تهيئة قاعدة البيانات، نكمل تشغيل البوت بدون قاعدة البيانات
        console.error('⚠️ سيتم تشغيل البوت بدون قاعدة البيانات بسبب فشل التهيئة');
    }
  ...
  // تهيئة نظام الألوان
  const refreshColor = (source) => {
    colorManager.forceUpdateColor().catch(err => {
      console.error(`❌ خطأ في تحديث ألوان الإيمبد (${source}):`, err);
    });
  };
  colorManager.initialize(client);
  setTimeout(() => refreshColor('startup'), 5000);
  setInterval(() => refreshColor('interval'), 10 * 60 * 1000);
  client.on(Events.ShardReady, () => refreshColor('shard_ready'));
  client.on(Events.ShardResume, () => refreshColor('shard_resume'));
  client.on(Events.ShardReconnecting, () => refreshColor('shard_reconnecting'));
  client.on(Events.ClientReady, () => refreshColor('client_ready'));
  client.on(Events.UserUpdate, (oldUser, newUser) => {
    if (newUser?.id === client.user?.id) {
      refreshColor('user_update');
    }
  });
