...
const { handleAdminApplicationInteraction } = require('./commands/setadmin.js');
...
async function handleDownDMInteraction(interaction, context) {
    ...
    try {
        ...
        // Handle other DM interactions similarly...
...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    if (interaction.customId && (interaction.customId.startsWith('admin_approve_') || interaction.customId.startsWith('admin_reject_'))) {
        console.log(`معالجة تفاعل التقديم الإداري: ${interaction.customId}`);
        
        try {
            await handleAdminApplicationInteraction(interaction);
        } catch (error) {
