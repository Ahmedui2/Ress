...
function createStatusEmbed(settings) {
    ...
    } else if (settings.approverType === 'role' && settings.approverTargets.length > 0) {
        approverDisplay = settings.approverTargets.map(id => `<@&${id}>`).join(', ');
    } else if (settings.approverType === 'responsibility' && settings.approverTargets.length > 0) {
...
module.exports = {
    ...
    async handleInteraction(interaction, context) {
        ...
        if (interaction.isStringSelectMenu() && customId === 'vac_resp_select') {
            ...
            vacations.settings.approverTargets = interaction.values; // Just one for now, as per user request
            saveVacations(vacations);
            const mainEmbed = createStatusEmbed(vacations.settings);
            await interaction.update({ content: '✅ Approver responsibility has been updated.', embeds: [mainEmbed], components: [] });
            return;
        }
    }
};
