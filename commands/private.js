...
function saveData(data) {
  try {
    ...
    console.error('private saveData error:', e);
  }
}
...
async function renderPanelGuideImage(guildId, backgroundUrl) {
  ...
  ctx.fillStyle = 'rgba(6, 9, 20, 0.65)';
  ctx.fillRect(0, 0, width, height);
  ...
  controls.forEach((c, i) => {
    ...
    ctx.fillText(`• ${c.emoji} ${c.label}: ${c.help}`, x, y);
  });

  const outPath = path.join(__dirname, '..', 'data', `private_panel_${guildId}.png`);
  fs.writeFileSync(outPath, canvas.toBuffer('image/png', { compressionLevel: 6 }));
  return outPath;
}
...
async function publishPanel(guild, state) {
  ...
  const imagePath = await renderPanelGuideImage(guild.id, state.panelBackgroundUrl || null);
...
async function handleInteraction(interaction, context = {}) {
  ...
  try {
    ...
    if (interaction.isModalSubmit() && interaction.customId === 'private_publish_panel_modal') {
      state.panelBackgroundUrl = interaction.fields.getTextInputValue('image_url').trim() || null;
